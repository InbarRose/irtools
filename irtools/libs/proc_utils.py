...
# logging
log = logging.getLogger('irtools.proc_utils')
...
def get_processes(name, log_id=None, **kwargs):
    ...
    kwargs.setdefault('trace_file', artifact_dir + '/get_processes.trace.out')
    ...
    write_file(artifact_dir + '/get_procs.out', collect_ret.append_output(log_id), mode='a')
    return procs
...
def get_proc_pids_by_cmd(cmd_part, **kwargs):
    ...
    :return:
    """
    trace_file = kwargs.pop('trace_file', artifact_dir + '/procs_by_cmd.txt')
...
def kill_process(kill_all=False, pid=None, name=None, cmd_part=None, mode='psutil', children_only=False, myself=False,
    ...
    if mode == 'cmdline':
        ...
        if ret.rc == 0 or ret.rc == 1 and ret.contains('No such process'):
            proc_was_killed = True
        write_file(artifact_dir + '/kill_proc.out', mode='a',
                   contents='KWARGS={}\nRET={}\n\n'.format(kwargs, ret))
